...
volatile uint32_t is_LoRa_joined = 0;
...
uint32_t getLoRaJoinStatus()
{
	return is_LoRa_joined;
}
...
void parseRecvStatus(char* buf) {
	...
	if ( strstr(buf, "+JOIN") != NULL ) {
		if ( strstr(buf, "failed") != NULL ) {
			is_LoRa_joined = LORA_STATUS_JOIN_ERROR;
			return;
		}
		if ( strstr(buf, "Done") != NULL ||
			strstr(buf, "Joined") != NULL ) {
			is_LoRa_joined = LORA_STATUS_JOIN_OK;
			return;
		}
	}
...
int loraSend(char* message) {
	...
	if ( is_LoRa_joined != LORA_STATUS_JOIN_OK )
	{
		return LORA_SENT_WAITING_JOIN;
	}
